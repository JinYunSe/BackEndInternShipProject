...
describe('Posts Controller Unit Test', () => {
  ...
  test('getPosts Method by Success', async () => {
    ...
    await postsController.getPost(mockRequest, mockResponse, mockNext);
    ...
    expect(mockResponse.json).toHaveBeenCalledWith(samplePosts);
  });
  ...
  test('createPost Method by Success', async () => {
    ...
    // 서비스 계층에 있는 createPost 메서드 실행 후 반환되는 데이터 형식
    ...
    const createdPost = await postsController.createPost(mockRequest, mockResponse, mockNext);
